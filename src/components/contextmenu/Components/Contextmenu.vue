...
export default {
  ...
  watch: {
    ...
    contextmenuReferences(contextmenuReferences) {
      ...
      this.contextmenuReferences &&
        this.contextmenuReferences.forEach(ref => {
          ref.el.addEventListener(this.eventType, () =>
            this.handleReferenceContextmenu(event, ref.binding.value)
          );
        });
  ...
  methods: {
    handleReferenceContextmenu(event, params) {
      ...
      const reference = this.contextmenuReferences.find(ref =>
    ...
    handleBodyClick(event) {
      const notOutside =
        ...
        (this.isClick &&
          this.contextmenuReferences.some(ref =>
            ref.el.contains(event.target)
          ));
      if (!notOutside) {
        this.visible = false;
