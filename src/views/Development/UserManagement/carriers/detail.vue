 ...
 * @LastEditTime: 2019-09-05 09:51:17
 * @LastEditors: Please set LastEditors
 -->
<template>
  <div>
    <common-wrapper>
      <sys-search class="search" @search="search" @rest="rest" />
    </common-wrapper>
        ...
        <a-list-item slot="renderItem" slot-scope="item, index">
            ...
            <a slot="title" href="javascript:;">
              ...
              <span>
                <el-button
                  ...
                  v-if="showBtn('handleDetail')"
                >
                  <a-icon :type="item.showDetail ? 'eye-invisible' : 'eye'" />
                </el-button>
                <el-button
                  v-if="item.approval.trim() == '审核中' && showBtn('handleCheck')"
          ...
          >
            <div class="detail_container">
              ...
              <div v-if="item.businessLicensePic">
                ...
                <span>
                  <a
                    v-if="
                      getFileType(item.businessLicensePic) == 'file' &&
                  ...
                  <img
                    v-if="
                      getFileType(item.businessLicensePic) == 'img' &&
                        item.businessLicensePic
                    "
              ...
              <div v-if="item.roadTrafficPermit">
                <span>道路交通运输许可证:</span>
                <span>
                  <a
                    v-if="
                      getFileType(item.roadTrafficPermit) == 'file' &&
                        item.roadTrafficPermit
                    "
                    :href="item.roadTrafficPermit"
                    >道路交通运输许可证</a
                  >
                  <img
                    v-if="
                      getFileType(item.roadTrafficPermit) == 'img' &&
                        item.roadTrafficPermit
                    "
                    :src="item.roadTrafficPermit"
                    alt=""
                    @click="showImg(item.roadTrafficPermit)"
                  />
                </span>
              </div>
              <div v-if="item.businessType">
                <span>业务类型:</span>
                <span>{{ item.businessType }}</span>
              </div>
              <div v-if="item.transportMode">
                <span>运输模式:</span>
              ...
              <div v-if="item.scopeOfTransport">
                <span>服务区域:</span>
                <span>{{ item.scopeOfTransport }}</span>
              </div>
            ...
            >
              ...
              <el-row class="check" v-if="item.approval.trim() == '审核中' && showBtn('handleCheck')">
                ...
                <el-col style="margin:1rem 0 " :span="24">
                  ...
                  <el-button type="warning" @click="check(item, 'no')"
                    >审核不通过</el-button
                  >
                  <el-button @click="checkCancel(index, item)">取消</el-button>
                </el-col>
              </el-row>
...
export default {
  ...
  methods: {
    ...
    check(item, result) {
      ...
      carrierApprove(params).then(res => {
        if (!res.error) {
          this.$message({
            message: "审核完成",
            type: "success"
