...
import { waybillLoading, waybillLoadingExport } from "@/api/datareport";
...
export default {
  ...
  methods: {
    ...
    waybillStatOfCarrier(params) {
      waybillLoading(params).then(res => {
        if (!res.error) {
    ...
    exportExcel(type,merchantid) {
      ...
      waybillLoadingExport({
        ...
        const fileName =
          "装配统计" + moment(new Date()).format("YYYYMMDD") + ".xls";
