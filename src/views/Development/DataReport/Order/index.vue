...
<template>
  <div class="distribute">
    ...
    <merchant
      ...
      :merchantTopData="merchantTopData"
    />
    <el-drawer
      title="商户统计"
      size="100%"
...
export default {
  ...
  methods: {
    ...
    waybillStatOfMerchant(params) {
      waybillTongji(params).then(res => {
        if (!res.error && res) {
          this.merchantData = res.list;
          this.merchantTopData = res.single;
        }
      });
    },
    gotoMerchant(row) {
      this.queryTomerchant({
        ...this.searchParams,
    ...
    //查看商户信息
    queryTomerchant(params) {
      queryTomerchant(params).then(res => {
        if (!res.error && res) {
          this.merchantData = res.list;
          this.merchantTopData = res.single;
        }
    ...
    exportExcel(type,merchantid) {
      ...
      waybillTongjiExport({
        ...
        const fileName =
          "订单统计" + moment(new Date()).format("YYYYMMDD") + ".xls";
