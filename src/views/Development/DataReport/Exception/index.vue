...
import { waybillAbnormal, waybillAbnormalExport } from "@/api/datareport";
...
export default {
  ...
  methods: {
    ...
    waybillStatOfCarrier(params) {
      waybillAbnormal(params).then(res => {
        if (!res.error) {
    ...
    exportExcel(type,merchantid) {
      ...
      waybillAbnormalExport({
       type:type,
        ...
        const fileName =
          "异常统计" + moment(new Date()).format("YYYYMMDD") + ".xls";
