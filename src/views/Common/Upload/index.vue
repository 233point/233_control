    ...
    >
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          容量支持:{{ fileSize }}MB( {{ fileSize * 1024 }}KB ), 文件格式:{{
            fileType
          }}, 一次最多可上传:{{ limitNum }}个
        </div>
      </el-upload>
      <h3 slot="title"><i class="el-icon-upload"></i>文件上传管理器</h3>
        ...
        <el-row>
          ...
          <el-col :span="10" :offset="isShowDown ? 0 : 14">
            ...
            <el-button
              ...
              >上传</el-button
            >
            <el-button @click="complete" :disabled="this.isSuccess"
              >完成</el-button
            >
          </el-col>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      uploadActionUrl: BASE_PER_FIX_URL + this.uploadFileUrl, // 上传url
      fileList: [], // 上传的文件列表
  ...
  methods: {
    ...
    // 下载文件
    downFile() {
      downloadFile({ pojo: this.pojo }).then(res => {
        if (res.url) {
