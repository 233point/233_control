    ...
    <el-table :data="saveParams.TsysExcelList" border :height="height">
      ...
      <el-table-column property="xlsSysColname" label="系统对应列">
        <template slot-scope="scope">
          <el-select
            v-model="scope.row.xlsSysColname"
            placeholder="请选择"
  ...
  methods: {
    ...
    handleData(params, options) {
      ...
      for (var i = 0; i < params.length; i++) {
        this.saveParams.TsysExcelList.push({
          xlsColid: params[i].xlsColid,
          xlsSysColname: option[i],
          xlsColname: params[i].xlsColname
        });
      }
    },
    getOptions(params) {
    ...
    changeOptions(newVal) {
      ...
      for (var i = 0; i < v1.length; i++) {
        ...
        for (var j = 0; j < v1.length; j++) {
          if (i === j) continue;
          if (v1[j].xlsSysColname === "" || v1[j].xlsSysColname == null)
            continue;
    ...
    save() {
      ...
      uploadFileSave(params).then(res => {
        this.cancel();
