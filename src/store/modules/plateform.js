...
import variables from "@/common/theme/theme-chalk.scss";

const SIDEBAR_EXPANDED_WIDTH = "180px"; // 展开宽度
const SIDEBAR_COLLAPSED_WIDTH = "60px"; // 折叠宽度
const HEADER_HEIGHT = "50px"; // 头部(header)高度
const TAGTAB_HEIGHT = "30px"; // 标签页(tagTabs)高度
// 侧边栏隐藏状态
const SIDEBAR_HIDDEN_STATE = Cookie.get(
  `${BASE_PER_FIX_KEY}_sidebar_hidden_state`
);
// 侧边栏折叠状态
const SIDEBAR_OPEN_STATE =
  Cookie.get(`${BASE_PER_FIX_KEY}_sidebar_open_state`) || "true";
...
const plateform = {
  state: {
    sidebarState: {
      ...
      width:
        SIDEBAR_HIDDEN_STATE === "true"
          ? "0px"
          : SIDEBAR_OPEN_STATE === "true"
          ? SIDEBAR_EXPANDED_WIDTH
          : SIDEBAR_COLLAPSED_WIDTH // 边栏宽度,elementUI 折叠变量为64px
    },
    ...
    theme: Cookie.get(`${BASE_PER_FIX_KEY}_theme_color`)
      ? "#" + Cookie.get(`${BASE_PER_FIX_KEY}_theme_color`)
      : variables.theme
  },
  ...
  mutations: {
    ...
    SET_COLLAPSE_SIDEBAR(state, sidebarStatus) {
      ...
      state.sidebarState.width = sidebarStatus
        ? SIDEBAR_EXPANDED_WIDTH
    ...
    SET_HIDDEN_SIDEBAR(state, isHidden) {
      ...
      state.sidebarState.width = isHidden
        ? "0px"
        : SIDEBAR_OPEN_STATE === "true"
        ? SIDEBAR_EXPANDED_WIDTH
        : SIDEBAR_COLLAPSED_WIDTH;
    ...
    ADD_SHORTCUTMENU(state, menuItem) {
      if (
        !state.shortcutMenu ||
        state.shortcutMenu.some(menu => menu.name === menuItem.name)
      )
        return;
    ...
    REMOVE_SHORTCUTMENU(state, menuItem) {
      state.shortcutMenu = state.shortcutMenu.filter(
        menu => menu.name !== menuItem.name
      );
      setLocalStorage(`${BASE_PER_FIX_KEY}_shortcut_menu`, state.shortcutMenu);
    },
    // 设置最大化操作区域(隐藏顶部)
  ...
  actions: {
    ...
    changeThemeVar({ commit }, data) {
      commit("CHANGE_ThemeVar", data);
    }
  }
};
