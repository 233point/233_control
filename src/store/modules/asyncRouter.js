...
function generateNewRoutes(remoteRoutes) {
  ...
  const _createRoutes = childRoute => {
    const newRoute = childRoute.map(route => {
      const isHasChildren = route.children && route.children.length;
      const addChildRoute = {
        path: isHasChildren ? "/" + route.path : route.path,
        name: route.name,
        component: _import_(route.component),
        meta: { ...route.meta, resourceUuid: route.menuUuid },
        resourceUuid: route.menuUuid
      };
      // 如果有子路由,则递归创建
      if (isHasChildren) {
    ...
    generateRouters: ({ commit }, roles) => {
      ...
      if (Vue.useStaticRouter) {
        return new Promise((resolve, reject) => {
          ...
          const addRoutes = filterAsyncRoutes(aysncRoutesMap, roles.roles);
        ...
        if (routerRawData) {
          ...
          return getGenerateRoutes({
            userUuid: roles.userUuid,
            sysType: "ctms"
          }).then(routerRawData => {
            const addRoutes = generateNewRoutes(routerRawData);
            commit("SET_ROUTERS", { addRoutes, routerRawData });
